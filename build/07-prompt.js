      ...
      function buildPrompt(){
        ...
        if (vis.style) {
          ...
          L.push(`Построй под этот стиль цельную визуальную систему: фон, карточки, кнопки, иконки, декор, шрифтовые акценты и атмосферу. Стиль должен быть узнаваем на каждом экране — стартовом, игровом, экране подсказки и финальном, а не только в цвете кнопок.`);
        } else {
          L.push('Стиль оформления выбери сам под тему урока и возраст учеников и выдержи его во всей игре.');
          L.push('Примеры того, что значит «выдержанный стиль»: космос — тёмный фон, звёзды, планеты, неоновые акценты; пиратский — карта, дерево, канаты, сундук, морские цвета; вязаный — мягкие текстуры, нити, пуговицы, тёплые пастельные тона; лаборатория — колбы, схемы, стекло, светлый фон; школьная тетрадь — клетка, синие чернила, поля, наклейки.');
        }
        ...
        L.push('В CSS создай отдельные переменные оформления игры и задай их под выбранный стиль: --game-bg, --game-card, --game-primary, --game-accent, --game-text, --game-success, --game-error, --game-muted. Не используй одну и ту же универсальную схему для разных стилей.');
      ...
      function update(){
        ...
        updateTimer = setTimeout(() => {
          ...
          el.size.textContent = `Длина промпта: ${text.length} символов, примерно ${words} слов.`;
          renderSummary();
          save();
        }, 120);
      }
