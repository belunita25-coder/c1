...
code = io.open('/home/user/c1/complex.html', encoding='utf-8').read()
...
html = f'''<title>Код конструктора сложных игр</title>
<style>
  :root {{
...
<div class="wrap">
  <header>
    ...
    <h1>Код конструктора сложных игр</h1>
    ...
    <div class="stats">
      ...
      <div class="stat"><b>2–8</b><span>этапов в игре</span></div>
      <div class="stat"><b>19+8</b><span>правил игры и блоков этапа</span></div>
      <div class="stat"><b>130</b><span>стилей с рецептами</span></div>
  ...
  <ol class="steps">
    <li><b>Как готовый файл.</b> Сохраните код в файл с именем <code>complex.html</code>
      и откройте двойным щелчком.</li>
    <li><b>Как блок на сайте.</b> Возьмите всё, что внутри
      <code>&lt;style&gt;…&lt;/style&gt;</code>, <code>&lt;main id="complex-prompt-constructor-iy"&gt;…&lt;/main&gt;</code>
      и <code>&lt;script&gt;…&lt;/script&gt;</code>, и вставьте в блок HTML-кода на странице.
      Все стили ограничены идентификатором <code>#complex-prompt-constructor-iy</code>
      и не влияют на остальной сайт.</li>
    <li><b>Чтобы дорабатывать.</b> Редактируйте фрагменты из папки <code>buildx/</code>
      и пересобирайте файл командой <code>sh buildx/build.sh</code> — так данные
      не перемешиваются с логикой.</li>
  </ol>

  <div class="note"><b>Где что менять.</b> Списки механик, вариантов ответа и стилей —
    это обычные массивы в начале скрипта: <code>MECHANICS</code>, <code>STAGE_PROMPTS</code>,
    <code>FEEDBACK</code>, <code>ENGAGEMENT</code>, <code>BLOCK_SCOPE</code> и <code>STYLE_GROUPS</code>.
  ...
  <h2>Весь код</h2>
  <div class="codebar">
    <span class="name">complex.html</span>
...
out = '/home/user/c1/buildx/code-page.html'
